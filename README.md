...
ソース中で関数や変数を記載することを**宣言**と呼び、各種関数や変数などをコンパイルしてメモリへ展開することを**定義**と呼ぶ。

##### extern 宣言

基本的に宣言と定義は同時に行われるため、複数ファイルで変数の宣言があると、複数回定義を行うためコンパイルエラーとなってしまう。それを避けるために、`extern宣言`が用意されている。
...
ヘッダーファイルで宣言しているように宣言の頭に`extern`を付与すると他ファイルで宣言内容を共有することができるようになる。

##### ifndef と endif

```C
// sum.h
#ifndef __INCLUDE_SUM_
#define __INCLUDE_SUM_
...
別ファイルで変数名が重複している場合、ヘッダーファイルで単一ファイルと同じように宣言してコンパイルすると定義が重複するためコンパイルエラーになる。そのため、すでに定義が行われている場合を想定する場合は`ifndef疑似命令`を利用する。`__INCLUDE_SUM_`が定義されていれば定義をスキップし、定義されていなければ定義を行う。そのため、関数や変数の重複定義を防ぐことができる。

#### ヘッダーファイルとソースコード

基本的に関数・変数の定義はヘッダーファイルで行い、関数の具体的処理をソースファイルへ記述する。基本的な開発の流れは、以下のようになる。

- ヘッダーファイルにて関数や変数を宣言
- ヘッダーファイルに宣言されている関数に沿ってコードの開発を行う
- 基本的にヘッダーファイルは設計書に近いものなので、ここに関数の機能説明などをコメントアウトで行うことが望ましい
